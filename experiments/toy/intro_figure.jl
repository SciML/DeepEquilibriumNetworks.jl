...
## Load Packages
using CUDA, Dates, DiffEqSensitivity, FastDEQ, Flux, CairoMakie, OrdinaryDiffEq,
      OrderedCollections, Statistics, SteadyStateDiffEq, Random, Zygote
...
function PolyFit(dims::Int, hdims::Int)
    return PolyFit(randn(Float32, hdims, dims), randn(Float32, dims, hdims),
                   randn(Float32, hdims, dims), zeros(Float32, hdims, 1))
end
...
function get_model(hdims::Int, abstol::T, reltol::T,
    ...
    return gpu((model_type == "vanilla" ? DeepEquilibriumNetwork :
                SkipDeepEquilibriumNetwork)(PolyFit(1, hdims),
                                            get_default_dynamicss_solver(abstol,
                                                                         reltol);
                                            sensealg=get_default_ssadjoint(abstol,
                                                                           reltol,
                                                                           20),
...
function train(model_type)
    ...
    data_size = 5096
    batch_size = 512
    x_data, y_data = gpu.(Flux.unsqueeze.(generate_polynomial_data(data_size),
                                          1))

    x_train, y_train = x_data[:, 1:(data_size - 1000)],
                       y_data[:, 1:(data_size - 1000)]
    x_val, y_val = x_data[:, (data_size - 1000 + 1):end],
                   y_data[:, (data_size - 1000 + 1):end]

    x_train_data_partition = (x_train[:,
                                      ((i - 1) * batch_size + 1):(i * batch_size)] for i in 1:(size(x_train, 2) ÷ batch_size))
    y_train_data_partition = (y_train[:,
                                      ((i - 1) * batch_size + 1):(i * batch_size)] for i in 1:(size(y_train, 2) ÷ batch_size))
    x_val_data_partition = (x_val[:,
                                  ((i - 1) * batch_size + 1):(i * batch_size)] for i in 1:(size(x_val, 2) ÷ batch_size))
    y_val_data_partition = (y_val[:,
                                  ((i - 1) * batch_size + 1):(i * batch_size)] for i in 1:(size(y_val, 2) ÷ batch_size))
...
function generate_trajectory(model::DeepEquilibriumNetwork, x::AbstractArray,
                             depth=50)
    traj = []
    u = zero(x)
    push!(traj, u)
...
function generate_trajectory(model::SkipDeepEquilibriumNetwork,
                             x::AbstractArray, depth=50)
    traj = []
    u = zero(x)
    push!(traj, u)
    for _ in 1:(depth - 1)
