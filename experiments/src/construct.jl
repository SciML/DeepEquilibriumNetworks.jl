...
function normal_initializer(rng::Random.AbstractRNG,
  dims...;
  mean::T=0.0f0,
...
function conv1x1(mapping,
  ...
  c = Lux.Conv((1, 1),
    ...
    activation;
    pad=0,
    init_weight=normal_initializer,
    stride,
    use_bias,
    dilation,
    groups)
...
function upsample_module(mapping,
  level_difference,
  activation;
  group_count=8,
...
struct ResidualBlock{C1, C2, Dr, Do, N1, N2, N3} <:
       Lux.AbstractExplicitContainerLayer{(:conv1,
  :conv2,
  :dropout,
  :downsample,
  :norm1,
...
function ResidualBlock(mapping;
  deq_expand::Int=3,
  num_gn_groups::Int=4,
  downsample=Lux.NoOpLayer(),
  n_big_kernels::Int=0,
...
function BottleneckBlock(mapping::Pair;
  expansion::Int=4,
  ...
  return BottleneckBlock(rescale,
    conv1x1(mapping),
    Lux.Chain(Lux.BatchNorm(last(mapping),
        NNlib.relu;
        affine=bn_affine,
        track_stats=bn_track_stats),
      conv3x3(last(mapping) => last(mapping)),
      Lux.BatchNorm(last(mapping),
        NNlib.relu;
        affine=bn_affine,
        track_stats=bn_track_stats),
...
function get_model(;
  ...
  downsample_layers = Lux.AbstractExplicitLayer[conv3x3(in_channels => init_channel_size;
      stride=(downsample_times >= 1 ? 2 : 1)),
    Lux.BatchNorm(init_channel_size, NNlib.relu; affine=true, track_stats=false),
    conv3x3(init_channel_size => init_channel_size; stride=(downsample_times >= 2 ? 2 : 1)),
    Lux.BatchNorm(init_channel_size, NNlib.relu; affine=true, track_stats=false)]
  ...
  main_layers = Tuple(ResidualBlock(num_channels[i] => num_channels[i];
    deq_expand=expansion_factor,
    dropout_rate=dropout_rate,
    num_gn_groups=group_count,
    n_big_kernels=big_kernels[i],
  ...
  for i in 1:num_branches, j in 1:num_branches
    ...
    elseif i < j
      mapping_layers[i, j] = downsample_module(num_channels[i] => num_channels[j],
        j - i,
        NNlib.relu;
        group_count)
    else
      mapping_layers[i, j] = upsample_module(num_channels[i] => num_channels[j],
        i - j,
        NNlib.relu;
        group_count,
  ...
  post_fuse_layers = Tuple(Lux.Chain(conv1x1(num_channels[i] => num_channels[i],
      NNlib.relu;
      weight_norm),
    Lux.BatchNorm(num_channels[i]; affine=true, track_stats=false)) for i in 1:num_branches)
  ...
  downsample_modules = [Lux.Chain(conv3x3(head_channels[i] * 4 => head_channels[i + 1] * 4;
        stride=2,
        use_bias=true),
      Lux.BatchNorm(head_channels[i + 1] * 4, NNlib.relu; affine=true, track_stats=false))
  ...
  final_layers = Lux.Chain(increment_modules,
    downsample_modules,
    conv1x1(head_channels[num_branches] * 4 => final_channelsize; use_bias=true),
    Lux.BatchNorm(final_channelsize, NNlib.relu; affine=true, track_stats=false),
    Lux.GlobalMeanPool(),
    Lux.FlattenLayer(),
    Lux.Dense(final_channelsize => num_classes);
    disable_optimizations=true)

  scales = compute_feature_scales(;
    image_size,
    downsample_times,
    num_channels,
    num_branches)
  ...
  if model_type in ("skip", "skipv2")
    if model_type == "skip"
      ...
      for i in 1:(num_branches - 1)
        push!(shortcut_layers,
          downsample_module(num_channels[1] => num_channels[i + 1],
            i,
            NNlib.relu;
            group_count))
      end
    ...
    deq = DEQ.MultiScaleSkipDeepEquilibriumNetwork(main_layers,
      mapping_layers,
      post_fuse_layers,
      shortcut,
      solver,
  ...
  elseif model_type == "neural_ode"
    ...
    deq = DEQ.MultiScaleNeuralODE(main_layers,
      mapping_layers,
      post_fuse_layers,
      solver.alg,
      scales;
    ...
    deq = DEQ.MultiScaleDeepEquilibriumNetwork(main_layers,
      mapping_layers,
      post_fuse_layers,
      solver,
      scales;
...
function construct(cfg::SolverConfig)
  if cfg.continuous
    ...
    return DEQ.ContinuousDEQSolver(solver;
      mode=Symbol(cfg.stop_mode),
      cfg.abstol,
      cfg.reltol,
      cfg.abstol_termination,
...
function construct(cfg::SensitivityConfig)
  return DEQ.DeepEquilibriumAdjoint(cfg.abstol,
    cfg.reltol,
    cfg.maxiters;
