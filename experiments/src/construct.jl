...
function get_model(;
    ...
    post_fuse_layers = Tuple(Lux.Chain(conv1x1(num_channels[i] => num_channels[i],
            NNlib.relu;
            weight_norm),
        Lux.BatchNorm(num_channels[i]; affine=true, track_stats=false))
    ...
    downsample_modules = [Lux.Chain(conv3x3(head_channels[i] * 4 => head_channels[i + 1] * 4;
                stride=2,
                use_bias=true),
            Lux.BatchNorm(head_channels[i + 1] * 4,
                NNlib.relu;
                affine=true,
