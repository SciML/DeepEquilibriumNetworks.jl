...
function main(config_name::String, cfg::DEQExperiments.ExperimentConfig)
    ...
    for step in initial_step:(cfg.train.total_steps)
        ...
        Setfield.@set! tstate.states = Lux.update_state(tstate.states,
            :update_mask,
            Val(true))
        ...
        if step % cfg.train.evaluate_every == 1 || step == cfg.train.total_steps
            ...
            for (x, y) in ds_val
                ...
                acc1, acc5 = DEQExperiments.accuracy(Lux.cpu(stats.y_pred),
                    Lux.cpu(y),
                    (1, 5))
            ...
            if DEQExperiments.should_log()
                ...
                Wandb.log(loggers.wandb_logger,
                    loggers.log_functions.eval(step, log_vals...))
