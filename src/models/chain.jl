...
function (deq::DEQChain{Val(5)})(x)
    x_, jac_loss = deq.deq(deq.pre_deq(x))
    return deq.post_deq(x_), jac_loss
end
...
function (deq::DEQChain{Val(6)})(x)
    x1 = deq.pre_deq(x)
    z, ẑ, jac_loss = deq.deq(x1)
    x2 = deq.post_deq(z)
    return x2, (z, ẑ), jac_loss
end
