...
function BasicResidualBlock(outdims::Tuple, inplanes::Int, planes::Int; deq_expand::Int=5, num_gn_groups::Int=4,
    ...
    conv1 = wn_layer((n_big_kernels >= 1 ? conv5x5 : conv3x3)(inplanes => inner_planes; stride=1, bias=false,
                                                              init=normal_init()))
    gn1 = GroupNormV2(inner_planes, num_gn_groups, gelu; affine=gn_affine, track_stats=gn_track_stats)

    conv2 = wn_layer((n_big_kernels >= 2 ? conv5x5 : conv3x3)(inner_planes => planes; stride=1, bias=false,
                                                              init=normal_init()))
    gn2 = GroupNormV2(planes, num_gn_groups, gelu; affine=gn_affine, track_stats=gn_track_stats)
...
function BasicBottleneckBlock(mapping::Pair, expansion::Int=4; bn_track_stats::Bool=true, bn_affine::Bool=true)
    ...
    chain = Chain(BatchNormV2(last(mapping), gelu; affine=bn_affine, track_stats=bn_track_stats),
                  conv3x3_norm(last(mapping) => last(mapping) * expansion, gelu; norm_layer=BatchNormV2,
                               conv_kwargs=Dict{Symbol,Any}(:bias => false, :init => normal_init()),
                               norm_kwargs=Dict{Symbol,Any}(:track_stats => bn_track_stats, :affine => bn_affine)).layers...,
