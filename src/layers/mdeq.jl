struct MultiScaleDeepEquilibriumNetwork{R,N,M1<:Parallel,M2<:Union{Chain,FChain},RE1,RE2,P,A,K,S} <:
    ...
    function MultiScaleDeepEquilibriumNetwork(residual_regularization::Bool, main_layers::Parallel, mapping_layers::Union{Chain,FChain}, re1, re2, p,
        ...
        return new{residual_regularization, length(ordered_split_idxs),
                   typeof.((main_layers, mapping_layers, re_main_layers, re_mapping_layers, p))...,A,K,S}(residual_regularization, main_layers,
                                                                                                          mapping_layers,
                                                                                                          re_main_layers,
                                                                                                          re_mapping_layers,
                                                                                                          p,
...
function MultiScaleDeepEquilibriumNetwork(main_layers::Tuple, mapping_layers::Matrix, solver;
    ...
    return MultiScaleDeepEquilibriumNetwork(residual_regularization, main_layers, mapping_layers, nothing, nothing, p,
                                            nothing, (solver,), kwargs, sensealg, DEQTrainingStats(0))
end

function (mdeq::MultiScaleDeepEquilibriumNetwork{false})(x::AbstractArray{T}) where {T}
    z = zero(x)
...
function (mdeq::MultiScaleDeepEquilibriumNetwork{true})(x::AbstractArray{T}) where {T}
    z = zero(x)
    initial_conditions = Zygote.@ignore map(l -> l(z), map(l -> l.layers[1], mdeq.mapping_layers[1].layers))
    u_sizes = Zygote.@ignore size.(initial_conditions)
    u_split_idxs = Zygote.@ignore vcat(0, cumsum(length.(initial_conditions) .÷ size(x, ndims(x)))...)
    ...
    return x_, sum(abs, dudt(res, mdeq.p, nothing))
end
